import { Button, Card, Flex, Heading, Image, Link, Spinner, Text } from "@chakra-ui/react";
  ...
  return (
    <>
      ...
      <Flex minH='100dvh' minW='100dvw' align='center' justify='center' p={4}>
        <Flex backdropFilter='blur(2px)' maxW='1440px' flexDir='column' gap={4} align='center'>
          {!loading && news && news.data.map((e) => <Card as={Link} href={e.url} target="_blank" key={e.uuid} display='flex' p={4} gap={6} direction={{ base: 'column', md: 'row' }} justify='center' align='center'>
            <Image src={e.image_url} h='200px' rounded='md' />
            <Flex direction='column' gap={4}>
              <Heading fontSize='xl' textOverflow='ellipsis'>{e.title}</Heading>
